...
const SidebarSB: React.FC<SidebarSBProps> = ({ activeTab, setActiveTab, onLogout, userEmail }) => {
  const [isSidebarToggled, setIsSidebarToggled] = useState(false);

  const menuItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'Dashboard', dividerAfter: true },
  ...
  return (
    <ul 
      className={`navbar-nav sidebar sidebar-dark accordion ${isSidebarToggled ? 'toggled' : ''}`} 
      id="accordionSidebar"
      style={{
    ...
    >
      {/* Sidebar - Brand */}
      <a className="sidebar-brand d-flex align-items-center justify-content-center" href="#" style={{ textDecoration: 'none', padding: '1.5rem 1rem', color: '#fff' }}>
        <div className="sidebar-brand-text mx-3" style={{ fontSize: '1.1rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05rem' }}>
          Comercial Marisol
        </div>
      </a>
